...
def test_validation_error_naive_datetime():
    ...
    with pytest.raises(ValueError, match="run_time must be timezone-aware"):
        DateTrigger(run_time=naive_dt)


def test_next_fire_time_future(future_date):
    """
    Test standard behavior: Scheduled time is in the future.
    Logic: Should return the exact run_time.
    """
    trigger = DateTrigger(run_time=future_date)
...
def test_next_fire_time_past(future_date):
    ...
    Logic: If run_time <= now, we missed it. Return None (don't run stale jobs).
    """
    trigger = DateTrigger(run_time=future_date)
...
def test_repr(future_date):
    ...
    Should output: DateTrigger(run_time=datetime(...))
    """
    trigger = DateTrigger(run_time=future_date)
